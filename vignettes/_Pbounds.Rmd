...
We can solve this problem with `Pbounds()`.
...
We want to know what's the probability that the car is behind the door we picked, No. 1; we can also ask about the probability for door No. 2 (which should be 1 minus the one for door No. 1), and, why not, also for door No. 3.
...
The target probability is thus `P(car1  ~  you1 & host3 & I)`

### Constraints

We must express the information we have and the game rules in terms of probabilities.
...
4. Finally, you have no reason to believe that the host favours one door over another, when a choice is possible, that is, when the car is behind the door you chose:
